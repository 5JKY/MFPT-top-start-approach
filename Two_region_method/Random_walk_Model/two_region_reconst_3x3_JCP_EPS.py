...
def add_legend(ax, loc="lower center", bbox_to_anchor=(0.45, 0.00)):
    legend = ax.legend(
        loc=loc,
...
def add_panel_label(ax, label, position="upper-left"):
    ...
    ax.text(x, y, label, transform=ax.transAxes,
            fontsize=16, ha=ha, va=va)

def double_gaussian_potential(x, A1=3, mu1=-1, sigma1=0.5,
                              A2=4, mu2=1, sigma2=0.6):
...
ari1_trans.steady_state = (
    ari1_trans.steady_state / (h * np.sum(ari1_trans.steady_state))
)
ira2_trans.steady_state = (
    ira2_trans.steady_state / (h * np.sum(ira2_trans.steady_state))
)
...
ax.plot(x1_arr, ari1_trans.steady_state, label="TM A", color="darkgreen")
ax.plot(x2_arr, ira2_trans.steady_state, label="TM B", color="darkorange")

ax.plot(n1_arr, two_Pst_n1, label="RW A", color="red",
        linestyle="--", linewidth=1.6, marker="o", markersize=4.5, markevery=15)
ax.plot(n2_arr, two_Pst_n2, label="RW B", color="blue",
        linestyle="--", linewidth=1.6, marker="o", markersize=4.5, markevery=15)
...
fig.savefig("two_Pst.eps", format="eps", bbox_inches="tight")
plt.close(fig)

# (b) MFPT
fig, ax = plt.subplots(figsize=(3.3, 2.75), constrained_layout=True)

ax.plot(x1_arr, trans_mfpt_n1, label="TM A", color="darkgreen")
ax.plot(x2_arr, trans_mfpt_n2, label="TM B", color="darkorange")

ax.plot(n1_arr, two_mfpt_n1, label="RW A", color="red",
        linestyle="--", linewidth=1.6, marker="o", markersize=4.5, markevery=15)
ax.plot(n2_arr, two_mfpt_n2, label="RW B", color="blue",
        linestyle="--", linewidth=1.6, marker="o", markersize=4.5, markevery=15)
...
# (c) Reconstructed free energy
fig, ax = plt.subplots(figsize=(3.3, 2.75), constrained_layout=True)

x_full = np.arange(b1, b2, h)

# Draw the model first so overlapping TM/RW curves remain visible.
ax.plot(
    x_full,
    beta_U(x_full),
    label="Model",
...
# TM reconstruction: dotted lines + small triangle markers.
ax.plot(
    x1_arr[1:-1],
    trans_reconst_n1,
    linestyle=":",
...
# RW reconstruction: dashed lines + circle markers on top.
ax.plot(
    n1_arr[1:-1],
    two_reconst_n1,
    label="RW A",
...
fig.savefig("two_reconst.eps", format="eps", bbox_inches="tight")
plt.close(fig)

print("Generated: two_Pst.eps, two_MFPT.eps, two_reconst.eps")
