...
plt.legend()
plt.savefig('graphs/reguera_double_well_potential.png', dpi=300)
plt.close()
...
np.save('data/reguera_Pst_n2', Pst_n2)
...
plt.legend()
plt.grid()
plt.savefig('graphs/reguera_Pst_simu.png', dpi=300)
plt.close()
...
plt.grid()
plt.legend()
plt.savefig('graphs/reguera_Pst_simu_ln.png', dpi=300)
plt.close()
...
mfpt2_simu_arr[0] = 0
np.save('data/reguera_mfpt_n2', mfpt2_simu_arr)
...
plt.legend()
plt.grid()
plt.savefig('graphs/reguera_MFPT_simu.png', dpi=300)
plt.close()
...
plt.grid()
plt.legend()
plt.savefig('graphs/reguera_Pst_exact.png', dpi=300)
plt.close()

# %%
plt.plot(x2_arr, -np.log(ira2_trans.steady_state), label="IRA-Pst")
plt.plot(x2_arr, -np.log(st2_P_arr), '--', label='num_integrate2', color='darkorange')
plt.xlabel('x')
plt.ylabel("$-ln[P_{st}(x)]$")
plt.grid()
plt.legend()
plt.savefig('graphs/reguera_Pst_exact_ln.png', dpi=300)
plt.close()
...
plt.legend()
plt.grid()
plt.savefig('graphs/reguera_MFPT_exact.png', dpi=300)
plt.close()
...
plt.legend()
plt.grid()
plt.savefig('graphs/reguera_reconst_exact.png', dpi=300)
plt.close()
...
plt.legend()
plt.grid()
plt.savefig('graphs/reguera_reconst_transfer.png', dpi=300)
plt.close()
...
np.save('data/reguera_reconst_n2', simu_beta_Grec2_arr2)
...
plt.legend()
plt.grid()
plt.savefig('graphs/reguera_reconst_simu.png', dpi=300)
plt.close()

# %% [markdown]
# ### Simplified MFPT formalism Reconstruction

# %%
beta_GrecM_arr = np.log((1-mfpt2_simu_arr/mfpt2_simu_arr[-1])/Pst_n2)
# const = (-np.log(Pst_n2))[2] - beta_GrecM_arr[2]
const = beta_U(x2_arr)[2] - beta_GrecM_arr[2]
beta_GrecM_arr += const

plt.plot(n2_arr, beta_GrecM_arr, label="ReAb_Malek reconst")
plt.plot(x2_arr[1:-1], beta_U(x2_arr[1:-1]), ':', label="exact")
plt.legend()
plt.grid()
plt.xlabel('n')
plt.ylabel('$ \\beta \Delta G(n) $')
plt.title('free energy reconstruction')
plt.savefig('graphs/reguera_reconst_malek.png', dpi=300)
plt.close()
...
plt.plot(n2_arr[1:-1], simu_beta_Grec2_arr2, label="ReAb_simu reconst")
plt.plot(n2_arr, beta_GrecM_arr, label="ReAb_Malek reconst")
plt.plot(x2_arr[1:-1], beta_U(x2_arr[1:-1]), ':', label="original")
# plt.plot(x2_arr, -np.log(ira2_trans.steady_state)+const, '--', label="-ln(Pst)")
...
plt.legend()
plt.grid()
plt.savefig('graphs/reguera_reconst_compare.png', dpi=300)
plt.close()
# %%
