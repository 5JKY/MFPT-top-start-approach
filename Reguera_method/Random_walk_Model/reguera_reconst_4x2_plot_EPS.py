...
def add_legend(ax):
    legend = ax.legend(
        loc="best",
        frameon=True,
        framealpha=1.0,  # EPS/PostScript does not support transparency
        fancybox=True,
        fontsize=16,
        handlelength=1.2,
        handletextpad=0.3,
...
format_axes(ax)
add_legend(ax)
add_panel_label(ax, "(a)", position="lower-left")
fig.savefig("reguera_Pst.eps", format="eps", bbox_inches="tight")
plt.close(fig)
...
fig.savefig("reguera_lnPst.eps", format="eps", bbox_inches="tight")
plt.close(fig)

# (c) MFPT
fig, ax = plt.subplots(figsize=(3.3, 2.75), constrained_layout=True)
ax.plot(x2_arr, delt_t * m2_bar[0], label="TM", color="darkorange")
...
fig.savefig("reguera_MFPT.eps", format="eps", bbox_inches="tight")
plt.close(fig)
...
fig.savefig("reguera_reconst.eps", format="eps", bbox_inches="tight")
plt.close(fig)

print("Generated EPS files: reguera_Pst.eps, reguera_lnPst.eps, reguera_MFPT.eps, reguera_reconst.eps")
