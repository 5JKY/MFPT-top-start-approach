...
Pst_n2 = np.load(
    "data/reguera_Pst_n2.npy"
)

mfpt2_simu_arr = np.load(
    "data/reguera_mfpt_n2.npy"
)
...
# Do not include the absorbing-boundary point where Pst = 0
...
trans_beta_GrecM_arr2 = np.log(
    (1 - m2_bar[0] / m2_bar[0, -1]) /
    ira2_trans.steady_state
)

const_trans = (
    beta_U(x2_arr)[2] -
    trans_beta_GrecM_arr2[2]
)

trans_beta_GrecM_arr2 += const_trans


simu_beta_Grec2_arr2 = np.load(
    "data/reguera_reconst_n2.npy"
)

simu_beta_GrecM_arr2 = np.log(
    (1 - mfpt2_simu_arr / mfpt2_simu_arr[-1]) /
    Pst_n2
)

const_simu = (
    beta_U(x2_arr)[2] -
    simu_beta_GrecM_arr2[2]
)
